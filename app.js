const policyItems = [
  {
    ...
    status: "Normative anchor",
    sourceUrl: "https://www.unesco.org/en/legal-affairs/recommendation-ethics-artificial-intelligence",
    summary: "Date and place of adoption for UNESCO's global AI ethics standard, applicable to Member States as a normative reference for responsible AI governance.",
    relevance:
      "Provides the principles that should structure Namibia's follow-up work: transparency, accountability, human oversight, fairness, and inclusion.",
    tags: ["AI ethics", "human rights", "governance"],
  },
  {
    date: "2022-09-07 - 2022-09-09",
    title: "Southern Africa AI cooperation forum hosted in Namibia",
    dimension: "Research",
    status: "Regional convening",
    sourceUrl: "https://www.unesco.org/en/node/70200",
    summary: "UNESCO Southern Africa sub-Regional Forum on AI convened ministers and senior experts from Southern African countries in Windhoek.",
    relevance:
      "Identified AI policy making, STEM education, scientific research, skills development, civil-service capacity, disaster-risk reduction, gender equality, and youth as relevant capacity areas.",
  ...
  },
  {
    ...
    sourceUrl: "https://www.unesco.org/en/articles/namibia-sets-roadmap-develop-ai-responsibly-and-ethically",
    summary: "Over 80 stakeholders from government, academia, civil society, and the private sector convened in Windhoek on responsible and ethical AI.",
    relevance:
      "The engagement links RAM follow-up to public AI literacy, data privacy and security, local cultures, local languages, Indigenous knowledge, and national framework development.",
  ...
  },
  {
    ...
    sourceUrl: "https://au.int/en/node/44004",
    summary: "African Union continental strategy published following Executive Council endorsement at its July 2024 session in Accra.",
    relevance:
      "Places Namibia's AI capacity-building work within a continental framework for ethical, responsible, equitable, and development-focused AI.",
  ...
  },
  {
    ...
    sourceUrl: "https://www.unesco.org/en/articles/namibia-launches-artificial-intelligence-readiness-assessment-report",
    summary: "UNESCO-supported assessment launched with NCRST, providing a national evidence base across legal, social, cultural, scientific, educational, economic, technical, and infrastructure dimensions.",
    relevance:
      "Highlights remaining capacity needs across governance, infrastructure, skills, education, public awareness, and responsible implementation.",
  ...
  },
  {
    ...
    sourceUrl: "https://www.unesco.org/en/articles/namibias-lawmakers-participate-unesco-led-training-artificial-intelligence-and-rule-law",
    summary: "UNESCO, KAS, NCRST, and partners trained 50 Namibian lawmakers at Gross Barmen on AI and the Rule of Law.",
    relevance:
      "Shows a concrete capacity-building pathway for policymakers and legal actors on ethical governance, human rights, data protection, access to information, due process, and cybersecurity.",
  ...
  },
  {
    ...
    sourceUrl: "https://www.unesco.org/ethics-ai/en/namibia",
    summary: "UNESCO's Namibia country profile notes the proposed National Responsible AI Institute and priority sectors for AI intervention.",
    relevance:
      "Supports the case for a coordination mechanism linking NCRST, universities, public institutions, firms, communities, and priority sectors.",
  ...
  },
  {
    ...
    dimension: "Inclusion",
    status: "Inclusion anchor",
    sourceUrl: "https://www.unesco.org/en/articles/namibia-sets-roadmap-develop-ai-responsibly-and-ethically",
    summary: "UNESCO's Namibia AI roadmap article records stakeholder concerns about culture, language, Indigenous knowledge, literacy, and public awareness.",
    relevance:
      "Justifies inclusion-sensitive capacity-building that reaches rural communities and addresses local-language access, consent, public trust, and awareness.",
...
const componentWeights = {
  digitalAccessGapIndex: 0.35,
  adultLiteracyGapIndex: 0.2,
  distanceIndex: 0.2,
  ruralShareIndex: 0.15,
  servicePressureIndex: 0.1,
};

const componentLabels = {
  digitalAccessGapIndex: "Digital access gap",
  adultLiteracyGapIndex: "Adult literacy gap",
...
let normalization = { maxPopulation: 1, maxDistanceKm: 1 };
...
async function initDashboard() {
  ...
  normalization = {
    maxPopulation: Math.max(...Object.values(actualIndicators).map((item) => item.raw.population2023)),
    maxDistanceKm: Math.max(...Object.values(actualIndicators).map((item) => item.raw.distanceFromWindhoekKm)),
  };
...
function renderPolicyList() {
  ...
  const filtered = policyItems.filter((item) => {
    ...
    const searchable = [item.title, item.dimension, item.status, item.summary, item.relevance, item.sourceUrl, ...item.tags]
      .join(" ")
      .toLowerCase();
        ...
        <article class="policy-card" data-dimension="${item.dimension}">
          <div class="policy-topline">
            ...
            <div class="policy-meta">
              ...
              <a class="source-link" href="${item.sourceUrl}" target="_blank" rel="noreferrer">Source</a>
            </div>
          </div>
          <p class="policy-relevance"><b>Capacity-building relevance</b>${item.relevance}</p>
          <div class="tags">
...
function renderRegionDetail(region) {
  ...
  const components = componentsForRaw(raw);
  regionDetail.innerHTML = `
    ...
    <ul class="detail-list">
      <li><b>Underlying indicators</b>
        ...
        digital access: ${raw.digitalAccessPct.toFixed(1)}%;
        adult literacy: ${raw.adultLiteracyPct.toFixed(1)}%;
...
function renderHeadline() {
  ...
  highestPriorityText.textContent = `${top.name} shows the strongest need signal in this view, based on digital access, adult literacy, distance from Windhoek, rural population share, and population pressure.`;
}
...
async function copyBriefingNote() {
  ...
  const note = `Namibia AI Capacity-Building Needs Assessment - summary note
...
function scoreFor(region) {
  const raw = actualIndicators[region.name]?.raw;
  if (!raw) return 0;
  const components = componentsForRaw(raw);
  return Math.round(Object.entries(componentWeights).reduce((total, [key, weight]) => total + components[key] * weight, 0));
}

function componentsForRaw(raw) {
  return {
    digitalAccessGapIndex: 100 - raw.digitalAccessPct,
    adultLiteracyGapIndex: 100 - raw.adultLiteracyPct,
    distanceIndex: (100 * raw.distanceFromWindhoekKm) / normalization.maxDistanceKm,
    ruralShareIndex: raw.ruralPopulationPct,
    servicePressureIndex: (100 * raw.population2023) / normalization.maxPopulation,
  };
}
